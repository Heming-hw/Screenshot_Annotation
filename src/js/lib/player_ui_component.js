...
module.exports = class PlayerUIComponent extends PlayerComponent {
  ...
  get $UI() {
    return Object.freeze({
      ...
      // // a list of template inputs:
      // templateInputs: this.$player.find('.vac-comment-showbox input'),

      // // a list of template labels (e.g. "Emboli Count (right): ")
      // templateLabels: this.$player.find('.vac-comment-showbox label')

      commentsContainer: $(".screenshot").find('.vac-comments-container'), // outer container for all comments
      controlElements: $(".screenshot").find('.vac-control'), // Each of multiple control elements in the control bar
      newCommentTextarea: $(".screenshot").find('.vac-video-write-new textarea'), // Textarea for writing a new comment
      timeline: $(".screenshot").find('.vjs-progress-control'), // Timeline element
      markerCursorHelpText: $(".screenshot").find('.vac-cursor-help-text'), // Help text that appears with 'click/drag..' on timeline
      controlBar: $(".screenshot").find('.vjs-control-bar'), // Conrol bar wrapper for vjs
      markerWrap: $(".screenshot").find('.vac-marker-wrap'), // wrapper element to place markers in on timeline
      coverCanvas: $(".screenshot").find('.vac-video-cover-canvas'), // Player cover during adding annotation state

      // a list of template inputs:
      templateInputs: $(".screenshot").find('.vac-comment-showbox input'),

      // a list of template labels (e.g. "Emboli Count (right): ")
      templateLabels: $(".screenshot").find('.vac-comment-showbox label')
    });
  }
  ...
  //   this.$screenImg.addClass('vac-disable-play')
  ...
  //   this.$screenImg.removeClass('vac-disable-play');
  // }

...
module.exports = class Controls extends PlayerUIComponent {
  constructor(bindArrowKeys, pluginObject) {
    super(pluginObject);
  ...
  bindEvents(bindArrowKeys) {
    ...
    // screenImag
    $(document)
      .on('click.vac-controls', '.vac-controls button', this.startAddNew.bind(this))
    // $(document)
    //   .on('click.vac-controls', '.vac-controls button', this.test.bind(this))
  
      /// original: if internal commenting
    // screenImag
    $(document)
